...
struct CheckoutPage: View {
    ...
    @State private var discountToast = false
    @State private var isLoadingApplying = false
    ...
    var body: some View {
        ZStack {
            VStack {
                ...
                PriceRow(title: "Discounts", desc: "\(String(format: "%.2f", Double(discountValue)! * rate)) \(currency)", color: .red)
                    .padding(.trailing, 8)
                
                PriceRow(title: "SubPrice", desc: "\(String(format: "%.2f", (Double(cartViewModel.subTotal ?? "0.0") ?? 0.0) * rate)) \(currency)")

                PriceRow(title: "Tax", desc: "\(String(format: "%.2f", (Double(cartViewModel.tax ?? "0.0") ?? 0.0) * rate)) \(currency)")

                PriceRow(title: "TotalPrice", desc: "\(String(format: "%.2f", (Double(cartViewModel.total ?? "0.0") ?? 0.0) * rate)) \(currency)")
            ...
            if discountToast {
                ...
                .animation(.easeInOut, value: discountToast)
            }
        }
        ...
        .onAppear {
            ...
            if UserDefaultManager.shared.isNotDefaultAddress == false {
    ...
    private func showDiscountToast() {
        withAnimation {
            discountToast = true
        }
    }
